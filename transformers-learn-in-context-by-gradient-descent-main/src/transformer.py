...
class Transformer(hk.Module):
  ...
  def trans_block(self, h, nl):
    ...
    # h is of (batch_size, sequence_length, embedding_size)
    ...
    else: #non-recurrent (more standard) case
      if nl == 0:
        h_norm = h
      else:
    ...
    if self.y_update: #If y_update is enabled, only the last token in the sequence is updated(autoregressive modelling)
