...
class Transformer(hk.Module):
  ...
  def __init__(
      ...
      num_layers: int = 3,
      key_size: int = 5,  #size of the K, Q, V vectors --> w_k, w_q, w_v are (embedding_size x key_size)
      embedding_size: int = 64, #length of a single token embedding
      output_size: int = 1,
      in_context_length: int = 17, #number of tokens per sequence
  ...
  def trans_block(self, h, nl):
    ...
    # nl is the current layer number 
    # h is (num heads, dataset_size + 1, input_size + output_size) = (1, 11, 2)
    if self.deq: #recurrent case
      h_norm = self.lnorm1(h) if self.use_layer_norm else h # should still be h since because we are not using layer norm
      if not self.include_query: # should be triggered
        key = h_norm[:, :-1, :] # this excludes the query
        value = h_norm[:, :-1, :] 
      else:
        key = h_norm
        value = h_norm

      # attnetion mechanism executed here 
      h_attn, att_map =self.attn_block(h_norm,key,value) # parameters are: query, key, value
    else: #non-recurrent case, shouldn't be triggered
      if nl == 0:
        h_norm = h
      else:
      ...
      h_attn, att_map = attn_block(h_norm, key, value) #using multihead attention to compute
    h_attn = hk.dropout(hk.next_rng_key(), self.dropout_rate, h_attn)

    if self.y_update: #If y_update is enabled, only the last token in the sequence is updated. Should be false (not triggered) here
      h = h.at[:, :, -1].set(h[:, :, -1] + self.dampening*h_attn[:, :, -1]) #QUESTION: if final token, shouldn't it be [h:, -1, :]? 
    else:
      h = h + self.dampening*h_attn # dampening should be 1.0, so doesn't really affect h_attn
    ...
    if not self.only_attention:
      ...
      if self.clip > 0:
        h = jnp.clip(h, -self.clip, self.clip)
    return h, att_map # basically returns the output of MHA since we are doing attention only
  ...
  def __call__(
    ...
    # shouldn't be triggered because self.vocab_size is its default value of 0
    ...
    # sets up a weight initializer
    self.w_init = hk.initializers.VarianceScaling(self.init_scale) # init_scale is 0.02
    self.dropout_rate = self.dropout_rate if is_training else 0.

    # final input mapping step, which will be h in later steps
    # should be false, as set in config, so not triggered
    if self.input_mapping:
      embeddings = hk.Linear(self.embedding_size,
                             with_bias=self.use_bias_p,
                             w_init=self.w_init, name="emb")(x)
    else:
      embeddings = x # this is triggered

    # should also not be triggered
    if self.input_mlp:
    ...
    # add / concat positional encodings to each member in batch(logic for test mode as well);
    # h is the embeddings if we are not adding positional encodings
    if self.use_pe:
      if self.concat_pe:
        if predict_test:
      ...
      h = embeddings # here, h ends up just being x, which are our tokens (jnp.ndarray)

    #obtain model size, but WHY IS IT DONE LIKE THIS??? 
    if len(h.shape) == 2:
    ...
    if self.deq:
      ...
      # shouldn't be triggered as self.only_attention is true right now
      if not self.only_attention:
        self.dense_block = MLP(w_init=self.w_init,
                               widening_factor=self.widening_factor,
                               use_bias_p=self.use_bias_p)
      # also shouldn't be triggered
      if self.use_layer_norm:
        self.lnorm1 = LNorm()
        self.lnorm2 = LNorm()

    # self.anacopy is defaulted to false, so we are not returning the full prediction stack, but we want the full prediction stack
    st = h[:, -1, -1]*(-1.0) if not self.ana_copy else (h if self.include_query else h[:, :-1, :]) 
    stack_h = [] if not self.input_mlp else [st]
    stack_att = []
    # passing through the layers now
    for nl in range(self.num_layers): 
      h, att_map = self.trans_block(h, nl) # get the output after a layer and the attention map
      # intermediate readout of test prediction
      st = h[:, -1, -1]*(-1.0) if not self.ana_copy else (h if self.include_query else h[:, :-1, :]) # would use the third option
      stack_h.append(st)
      stack_att.append(att_map)
    out = hk.Linear(self.output_size)(h) if self.output_mapping else h # would just give h since we are not using output mapping

    if self.return_logits:
      out = vocab(out, logits=True)
      # out: Final output after applying the attention mechanism and any optional projection.
      # stack_h: Hidden states or predictions at each layer of the Transformer.
      # stack_att: Attention maps for each layer, showing the attention weights for each query-key pair.
