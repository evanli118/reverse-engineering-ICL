...
class TokenVocab(hk.Module):
  ...
  def __call__(self, x, logits=False):
    vocab = hk.get_parameter("vocab", [self.vocab_size, 1, self.e_size], init=self.w_init) #vocab is a trainable parameter
    if logits:
...
class MultiHeadAttention(hk.Module):
  ...
  def __call__(
    ...
    *leading_dims, sequence_length, _ = query.shape # sequence length should be 11
    ...
    return attn, attn_weights  # attn has shape (1, 11, 2), attn_weights has shape (1, 1, 11, 10)


  '''
    The _linear_projection function serves to apply a linear transformation to an input tensor x, 
    producing outputs structured as multiple attention heads. It initializes a linear layer 
    using parameters like num_heads * head_size and optionally includes biases. T
    he reshaping step organizes the output into a tensor format suitable for subsequent operations in multi-head attention mechanisms, 
    commonly used in transformer architectures for tasks like natural language processing.
  '''
  @hk.transparent
